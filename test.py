...
def guardar_con_formato_original(df_procesado, archivo_original_bytes, nombre_hoja, fila_cabecera, agregar_columnas_nuevas=False):
    ...
    if agregar_columnas_nuevas:
        # Leer cabecera actual del Excel
        cabecera_actual = [cell.value for cell in ws[fila_cabecera_excel]]
        cabecera_df = list(df_procesado.columns)
        ...
        # Agregar las columnas nuevas al final de la cabecera
        if columnas_nuevas:
            ultima_col = len(cabecera_actual)
            for idx, nueva_col in enumerate(columnas_nuevas, start=1):
                ws.cell(row=fila_cabecera_excel, column=ultima_col + idx, value=nueva_col)
